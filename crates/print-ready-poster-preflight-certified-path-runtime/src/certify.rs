...
pub fn certify_print_preflight(
    ...
    let artifacts = vec![
        artifact("source_svg", ArtifactRole::Input, request.source_uri, Some("image/svg+xml"), &sha256_hex(request.source_svg), byte_len(request.source_svg)),
        artifact("print_specification", ArtifactRole::Input, request.specification_uri, Some("application/json"), &sha256_hex(&specification_bytes), byte_len(&specification_bytes)),
        artifact("editable_master", ArtifactRole::Output, request.editable_uri, Some("image/svg+xml"), &sha256_hex(request.editable_svg), byte_len(request.editable_svg)),
        artifact("raw_inkscape_pdf", ArtifactRole::Evidence, "evidence://print/raw-inkscape.pdf", Some("application/pdf"), &sha256_hex(request.raw_pdf), byte_len(request.raw_pdf)),
        artifact("delivery_pdf", ArtifactRole::Output, request.delivery_uri, Some("application/pdf"), &sha256_hex(request.delivery_pdf), byte_len(request.delivery_pdf)),
        artifact(EXECUTION_EVIDENCE_ID, ArtifactRole::Evidence, "evidence://print/execution-record.json", Some("application/vnd.ergaxiom.signed-print-preflight-execution+json"), &sha256_hex(&execution_bytes), byte_len(&execution_bytes)),
        artifact(VALIDATION_EVIDENCE_ID, ArtifactRole::Evidence, "evidence://print/validation-report.json", Some("application/vnd.ergaxiom.print-preflight-validation+json"), &sha256_hex(&validation_bytes), byte_len(&validation_bytes)),
    ];
    ...
    if assessment.decision.status != DecisionStatus::Accepted {
        return Err(PrintPreflightCertificationError::EvidenceDecisionNotAccepted(
...
fn proof_results(
    ...
    vec![
        proof("restricted-svg", "restricted_svg_profile", "source_svg", "print.svg.structure", json!(report.restricted_svg_profile), json!(true), None, evaluated_at),
        proof("canvas", "canvas_dimensions_match", "source_svg", "print.canvas.dimensions", json!(report.canvas_dimensions_match), json!(true), None, evaluated_at),
        proof("bleed", "bleed_coverage", "source_svg", "print.bleed.coverage", json!(report.bleed_coverage), json!(true), None, evaluated_at),
        proof("safe-area", "safe_area_satisfied", "source_svg", "print.safe_area.geometry", json!(report.safe_area_satisfied), json!(true), None, evaluated_at),
        proof("palette", "palette_violations", "source_svg", "print.palette.allowlist", json!(report.palette_violation_count), json!(0), Some("count"), evaluated_at),
        proof("vector", "vector_only", "delivery_pdf", "print.pdf.vector_only", json!(report.vector_only), json!(true), None, evaluated_at),
        proof("fonts", "fonts_outlined", "delivery_pdf", "print.pdf.fonts_outlined", json!(report.fonts_outlined), json!(true), None, evaluated_at),
        proof("pages", "page_count", "delivery_pdf", "print.pdf.page", json!(report.page_count), json!(1), Some("page"), evaluated_at),
        proof("media-box", "media_box_match", "delivery_pdf", "print.pdf.boxes", json!(report.media_box_match), json!(true), None, evaluated_at),
        proof("trim-box", "trim_box_match", "delivery_pdf", "print.pdf.boxes", json!(report.trim_box_match), json!(true), None, evaluated_at),
        proof("bleed-box", "bleed_box_match", "delivery_pdf", "print.pdf.boxes", json!(report.bleed_box_match), json!(true), None, evaluated_at),
        proof("crop-box", "crop_box_match", "delivery_pdf", "print.pdf.boxes", json!(report.crop_box_match), json!(true), None, evaluated_at),
        proof("pdf-version", "pdf_version", "delivery_pdf", "print.pdf.version", json!(report.pdf_version), json!("1.5"), None, evaluated_at),
        proof("color-space", "allowed_color_spaces", "delivery_pdf", "print.pdf.color_spaces", json!(report.allowed_color_spaces_only), json!(true), None, evaluated_at),
        proof("transparency", "transparency_absent", "delivery_pdf", "print.pdf.transparency", json!(report.transparency_absent), json!(true), None, evaluated_at),
        proof("security", "external_actions_absent", "delivery_pdf", "print.pdf.security", json!(report.external_actions_absent), json!(true), None, evaluated_at),
        proof("source-immutable", "source_immutable", "source_svg", "print.source.immutability", json!(report.source_immutable), json!(true), None, evaluated_at),
        proof("inkscape", "inkscape_export_verified", "delivery_pdf", "print.inkscape.integration", json!(report.inkscape_export_verified), json!(true), None, evaluated_at),
    ]
}
