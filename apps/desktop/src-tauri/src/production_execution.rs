...
impl ProductionExecutionState {
    ...
    /// Compatibility boundary for the original desktop-shell lifecycle. Product Alpha must call
    ...
    pub fn with_fresh_lease_for_job<R>(
        ...
        let mut authority = match runtime.authorities.remove(job_id) {
            ...
            None => PersistentProductionExecutionAuthority::load_or_create(
                runtime.policy_store_root.clone(),
                runtime.execution_store_root.join(job_id),
                job_id,
...
impl ProductionExecutionRuntime {
    fn initialize() -> Result<Self, ProductionExecutionBoundaryError> {
        ...
        let desktop_authority = PersistentProductionExecutionAuthority::load_or_create(
            ...
            execution_store_root.join(DESKTOP_JOB_ID),
            DESKTOP_JOB_ID,
            executor_id,
            device_id,
        )?;
